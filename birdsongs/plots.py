...
class Ploter(object): 
    ...
    def Plot(self,obj, syllable_on=False, chunck_on=False, FF_on=False): 
        if "song" in obj.id:
            ...
            ax[0].pcolormesh(obj.tu, obj.fu*1e-3, obj.Sxx, cmap=plt.get_cmap('Greys'), rasterized=True)
            if FF_on: 
                ax[0].plot(obj.timeFF, obj.FF*1e-3, "co", label="Whole Song", ms=5)
                ax[0].legend()
            ...
            ax[0].set_ylim(np.array(obj.flim)*1e-3); ax[0].set_xlim(min(obj.time), max(obj.time));
            ...
            if chunck_on and syllable_on:
                ax[0].plot(obj.chunck.timeFF+obj.chunck.t0, obj.chunck.FF*1e-3, 'gv', label='Chunck', ms=10)
                ax[2].plot(obj.chunck.timeFF+obj.chunck.t0, obj.chunck.FF*1e-3, 'gv', label='Chunck', ms=8)

            if syllable_on:
                ax[0].plot(obj.syllable.timeFF+obj.syllable.t0, obj.syllable.FF*1e-3, 'b+', label='Syllable'.format(obj.syllable.fs), ms=6)
                ...
                ax[2].plot(obj.syllable.timeFF+obj.syllable.t0, obj.syllable.FF*1e-3, 'b+', label='Syllable', ms=15)
                ax[2].set_ylim((2, 11)); 
            ...
            ax[0].pcolormesh(obj.tu+obj.t0, obj.fu*1e-3, obj.Sxx, cmap=plt.get_cmap('Greys'), rasterized=True)
            ax[0].plot(obj.timeFF+obj.t0, obj.FF*1e-3,"bo", ms=7,label="Fundamental Frequency")
            ax[0].legend()
            ax[0].set_ylim(np.array(obj.flim)*1e-3); ax[0].set_xlim(min(obj.time)+obj.t0, max(obj.time)+obj.t0);
            ax[0].set_title("Spectrum"); 
            ax[0].set_ylabel('f (kHz)'); #ax[0].set_xlabel('t (s)'); 


            ax[1].plot(obj.time+obj.t0, obj.s/np.max(obj.s),'k', label='audio')
            ax[1].plot(obj.time+obj.t0, obj.envelope, label='envelope')
            ax[1].legend(loc='upper right')#loc=1, title='Data')
    ...
    def Result(self, obj, obj_synth, cmp="afmhot_r"):
        if not("synth" in obj.id) and "synth" in obj_synth.id:
            ...
            ax12.plot(obj_synth.FF_time, obj_synth.deltaSCI, 'ko-', label=r'$||SCI||_{}$={:.4f},'.format(obj.ord, obj_synth.scoreSCI)+'\n   mean={:.3f}'.format(obj_synth.deltaSCI_mean))

            ax12.plot(obj.FF_time, obj_synth.correlation, 'p-', label=r'$||cor||_{}$={:.3f},'.format(obj.ord, obj_synth.scoreCorrelation)+'\n   mean={:.3f} '.format(obj_synth.correlation.mean()))
            ax12.plot(obj.FF_time, obj_synth.Df, 'H-', label=r'$||DF||_{}$={:.3f},'.format(obj.ord, obj_synth.scoreDF)+'\n  mean={:.3f} '.format(obj_synth.Df.mean()))
            ax12.plot(obj.FF_time, obj_synth.SKL, 's-', color="tomato", label=r'$||SKL||_{}$={:.3f},'.format(obj.ord, obj_synth.scoreSKL)+'\n  mean={:.3f}'.format(obj_synth.SKL.mean()))
