...
for i in range(len(freq)):
    if freq[i] > 100:
        ...
        ap_alfa[i] = -0.05
        ap_beta_snilc[i] = f_q_snilc(freq[i])
    else:
        ap_beta[i] = -0.15
...
fig, ax = plt.subplots(3, 3, figsize=(18, 6), sharex='col', sharey='col')
...
ax[1][0].legend()
ax[2][0].plot(time, normalizar(out_snilc),
              label='snilc gamma {}'.format(gamma_snilc))
ax[2][0].plot(time, s_amp_env_snilc)
ax[2][0].legend()
...
ax[1][1].pcolormesh(tu_s, fu_s, np.log(Sxx_s), cmap=plt.get_cmap('Greys'),
                    rasterized=True)
ax[2][1].pcolormesh(tu_s, fu_s, np.log(Sxx_s_snilc),
                    cmap=plt.get_cmap('Greys'),
                    rasterized=True)
ax[2][1].set_ylim(0, 8000)
ax[2][1].set_xlim(0, 6.5)

ax[0][2].pcolormesh(tu, fu, np.log(Sxx), cmap=plt.get_cmap('Greys'),
                    rasterized=True)
ax[1][2].pcolormesh(tu_s, fu_s, np.log(Sxx_s), cmap=plt.get_cmap('Greys'),
                    rasterized=True)
ax[2][2].pcolormesh(tu_s, fu_s, np.log(Sxx_s_snilc),
                    cmap=plt.get_cmap('Greys'),
                    rasterized=True)
ax[2][2].set_ylim(0, 8000)
ax[2][2].set_xlim(2.8, 5.05)
...
wavfile.write('{}-snilc_gamma{}.wav'.format(out_name, gamma_snilc), fs,
