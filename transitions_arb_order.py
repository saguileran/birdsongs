    ...
    for n in range(len(all_wavs)):
        ...
        for song_aux in df_log['song'][df_log['File name'] == ff]:
            ...
            if fin > n_order:
                ...
                n_tr_matrix[all_wavs.index(ff)][i_order][row][column] += 1
        p_tr_matrix[all_wavs.index(ff)][i_order] = \
            normalize(n_tr_matrix[all_wavs.index(ff)][i_order],
                      axis=1, norm='l1')
        reps = np.diag(np.diag(n_tr_matrix[all_wavs.index(ff)][i_order]))
        n_tr_matrix_norep[all_wavs.index(ff)][i_order] = \
            n_tr_matrix[all_wavs.index(ff)][i_order] - reps
        p_tr_matrix_norep[all_wavs.index(ff)][i_order] = \
            normalize(n_tr_matrix_norep[all_wavs.index(ff)][i_order],
                      axis=1, norm='l1')
        fig, ax = plt.subplots(ncols=2, figsize=(16, 8))
        cax = ax[0].matshow(p_tr_matrix[all_wavs.index(ff)][i_order],
                            cmap=plt.cm.Blues)
        ...
        ax[0].set_xlabel('To')
        ax[0].set_title('Transiciones de orden {}, tipo: {}'.format(n_order, order_hier))
        cax = ax[1].matshow(p_tr_matrix_norep[all_wavs.index(ff)][i_order],
                            cmap=plt.cm.Blues)
...
# %% Testear
dtime = 3
groups_index = [np.where(np.logical_and(hours-x <= dtime, hours > x))[0]
                for x in hours[:-1]]
